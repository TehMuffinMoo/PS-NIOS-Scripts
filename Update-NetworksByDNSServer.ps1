...
function Set-NIOSNetworksDNS {
    ...
    $Objects | Select-Object network,network_view,@{name='dns_servers';expr={($_.options | Where-Object {$_.name -eq "domain-name-servers"}).value -join ","}} | ft -AutoSize
...
Set-NIOSNetworksDNS $NetworksWithOldDNS $OldIPs '10.255.255.1','10.255.255.2' -DryRun